...
#' @param preferences A character string to specify for which econ. preference should effect sizes be calculated. 
...
# EFFECT SIZES ----------------------------------------------------------
  ...
  cor_dat_av_grs <- df1 %>% # For correlations from groups
    filter(!is.na(age_avoidance_group_1_result))
  
  cor_dat_av_grs$ri = c(cor_dat_av_grs$age_avoidance_group_1_result,
                                               cor_dat_av_grs$age_avoidance_group_2_result)
  cor_dat_av_grs$ni = c(cor_dat_av_grs$total_n_group_1,
                                               cor_dat_av_grs$total_n_group_2)
  
  cor_dat_av_grs <- escalc(measure = 'COR',
                    ri = ri,
                    ni = ni,
                    vtype = "LS",
                    data = cor_dat_av_grs)
  
  cor_dat_av_gds <- df1 %>% # For correlations on genders
    filter(!is.na(female_age_avoidance_result)) 
  
  cor_dat_av_gds$ri = c(cor_dat_av_gds$female_age_avoidance_result,
                                               cor_dat_av_gds$male_age_result)
  cor_dat_av_gds$ni = c(cor_dat_av_gds$total_n_men,
                                               cor_dat_av_gds$total_n_women)
    
  cor_dat_av_gds <- escalc(measure = 'COR',
                    ri = ri,
                    ni = ni,
                    vtype = "LS",
                    data = cor_dat_av_gds)
  ...
  es_av <- bind_rows(smd_dat_av, smd_dat_av_ado, cor_dat_av, #cor_dat_av_grs,
                     cor_dat_av_gds) %>% 
  ...
  cor_dat_dp_gds <- df1 %>% # for correlations on genders
    filter(!is.na(female_age_dependence_result)) 
  
  cor_dat_dp_gds$ri = c(cor_dat_dp_gds$female_age_dependence_resultt,
                                               cor_dat_dp_gds$male_age_dependence_result)
  cor_dat_dp_gds$ni = c(cor_dat_dp_gds$total_n_men,
                                               cor_dat_dp_gds$total_n_women)
  
  cor_dat_dp_gds <- escalc(measure = 'COR',
                    ri = ri, 
                    ni = ni,
                    vtype = "LS",
                    data = cor_dat_dp_gds)
  ...
  es_dp <- bind_rows(smd_dat_dp, cor_dat_dp_gds, cor_dat_dp) %>% 
    select(study_id, yi, vi, cor_type, age_variable_study_1, dec_diff) %>% 
    rename(cor_yi_dp = yi,
           cor_vi_dp = vi) %>% 
    full_join(df1, by = c("study_id", "age_variable_study_1"))  %>% 
   ...
   es_dat <- es %>% 
      ...
      select(study_id, study_id_1, year_of_publication, country, measure, analysis,
             ...
             total_n_group_1, total_n_group_2, total_n_men, total_n_women, 
             n_incl_es_av, n_incl_es_dp, gender_female_value,
             gender_female_group_1, gender_female_group_2, age_m_young_adults, 
             age_m_older_adults, age_m_value, age_m_group_1, age_m_group_2, age_m_men, age_m_women,
...
preferences <- c("avoidance", "dependence")
