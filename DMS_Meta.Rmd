...
df1$sample_type_1 <- gsub('Majority community', 'Community', df1$sample_type_1)

df1$sample_type_1 <- gsub('Convenience - Online study', 'Convenience', df1$sample_type_1)
...
dat$sample_type_1 <- as.factor(dat$sample_type_1)

dat$sample_type_1 <- ordered(dat$sample_type_1, levels = c("University students", "Community", "Convenience", "Professional"))
...
for (rho in rho_vec) {
  ...
  m[[10]] <- rma.mv(yi ~ 1 + year_of_publication,
                   V = V_mat,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=dat,
                   digits=4)
  ...
  m[[11]] <- rma.mv(yi ~ 1 + sample_type_1,
                   V = V_mat,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=dat,
                   digits=4)
...
multiplemoderator <- rma.mv(yi ~ 1 + sample_type_1,
                   V = V_mat,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=dat,
                   digits=4)
