 ...
 # if (preference == "avoidance") {
  ...
  cor_dat_av <-  cor_dat_av %>% 
   ...
   select(study_id, yi, vi, cor_type, age_variable_study_1, dec_diff_av) %>% 
    ...
    mutate(author_extract = "Leon",
      es_id_av = 1:n(),  
          ...
          n_incl_es_av = case_when(analysis == "Correlation" ~ total_number_of_participants_study_1,
   ...
   # Reverse effect sizes
      es_av <- es_av %>% 
        rowwise() %>% 
        mutate(reversed_es_av = case_when(!is.na(young_adults_avoidance_mean) 
                                          & !is.na(older_adults_avoidance_mean) &
                                            young_adults_avoidance_mean > older_adults_avoidance_mean ~ 1,
                                          !is.na(avoidance_age_result) & avoidance_age_result < 0 ~ 1,
                                          TRUE ~ 0))
      
      es_av <- es_av %>%
        mutate(cor_yi_av = ifelse(reversed_es_av == 1, cor_yi_av * -1, cor_yi_av))
...
#  if (preference == "dependence") {
  ...
  es_dp <- bind_rows(smd_dat_dp, cor_dat_dp) %>% 
    ...
    mutate(author_extract = "Leon",
           es_id_dp = 1:n(),
  # number of participants included in the ES calculation
           n_incl_es_dp = case_when(analysis == "Correlation" ~ total_number_of_participants_study_1,
                                 # extreme group comparison included old vs. young differences
                                 analysis!= "Correlation" ~ number_of_older_adults_categorical_var_study_1 + number_of_young_adults_categorical_var_study_1))
     ...
     # Reverse effect sizes
      es_dp <- es_dp %>% 
        rowwise() %>% 
        mutate(reversed_es_dp = case_when(!is.na(young_adults_dependence_mean) 
                                          & !is.na(older_adults_dependence_mean) &
                                            young_adults_dependence_mean > older_adults_dependence_mean ~ 1,
                                          !is.na(dependent_age_result) & dependent_age_result < 0 ~ 1,
                                          TRUE ~ 0))
      
      es_dp <- es_dp %>% 
        mutate(cor_yi_dp = case_when(reversed_es_dp == 1  ~ cor_yi_dp*-1, 
                                reversed_es_dp == 0 ~cor_yi_dp))
...
#  }
   ...
   es_dat_av <- es_av %>% 
      ...
      select(study_id, study_id_1, title, year_of_publication, country, measure, analysis,
             ...
             dec_diff_av, young_adults_avoidance_mean, young_adults_avoidance_sd,
             older_adults_avoidance_mean, older_adults_avoidance_sd,
             avoidance_age_result, cor_yi_av, cor_vi_av, cor_type,
             es_id_av, n_incl_es_av, reversed_es_av, author_extract)
     ...
     es_dat_dp <- es_dp %>% 
      ...
      select(study_id, study_id_1, title, year_of_publication, country, measure, analysis,
             ...
             culture_uncertaintyavoidance, culture_longtermorientation, culture_longtermorientation,
             culture_indulgence, topic, sample_type, groups, analysis, total_number_of_participants_study_1,
             number_of_older_adults_categorical_var_study_1, number_of_young_adults_categorical_var_study_1, gender_female_value,
             age_m_young_adults, age_m_older_adults, age_m_value, 
             min_age_young, min_age_older, min_age, 
             max_age_young, max_age_older, max_age, 
             ...
             es_id_dp, n_incl_es_dp, reversed_es_dp, author_extract)
  
     all_es_dat <- es_dat_av %>% left_join(es_dat_dp)
     
     all_es_dat$study_id_2 = all_es_dat$study_id
     
     all_es_dat <- all_es_dat %>% 
...
table_es_overview <- function(dat) {
  ...
  dat_n <- dat %>% 
    distinct(study_id, n_incl_es_av, n_incl_es_dp, pub_study) %>% 
    group_by(study_id) %>%  
    ...
    filter(n_incl_es_av == max(n_incl_es_av)) %>% 
    ungroup() 
  ...
  t <-  tibble(
    ...
    n_es = nrow(dat),
    n_participants_avoidance = sum(dat_n$n_incl_es_av),
    n_participants_dependence = sum(dat_n$n_incl_es_dp),
...
dat_av1 <- dat %>% 
  ggplot(aes(x= cor_yi_av)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Estimate")

dat_av2 <- dat %>% 
  ggplot(aes(x= cor_vi_av)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Estimate")

dat_av3 <- dat %>% 
  ggplot(aes(x= log10(n_incl_es_av))) + 
  geom_density() +
  theme_bw() +
  ggtitle("Sample")

dat_av4 <- dat %>% 
  ggplot(aes(x= dec_diff_av)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Sample age Differences (decades)")

dat_dp1 <- dat %>% 
  ggplot(aes(x= cor_yi_dp)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Estimate")

dat_dp2 <- dat %>% 
  ggplot(aes(x= cor_vi_dp)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Estimate")

dat_dp3 <- dat %>% 
  ggplot(aes(x= log10(n_incl_es_dp))) + 
  geom_density() +
  theme_bw() +
  ggtitle("Sample")

dat_dp4 <- dat %>% 
  ggplot(aes(x= dec_diff_dp)) + 
  geom_density() +
  theme_bw() +
  ggtitle("Sample age Differences (decades)")
