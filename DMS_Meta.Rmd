...
for (rho in rho_vec) {
  ...
  m[[10]] <- rma.mv(yi ~ 1 + sample_type_1,
                   V = V_mat,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=dat,
                   digits=4)
...
### Separate analyses on participant sample type
```{r}

rho = .5

table(dat$sample_type_1)


types <- c("Community", "Convenience", "Professional", "University students")

for (typ in types) {
  
  # filter effect size datasets
  sdat <- dat %>% filter(sample_type_1 %in% typ) 
  
  
  # fitting multilevel model
  mlma_mod <- conduct_mlma(dat = sdat, rho = rho)
  mlma_mod <- robust(mlma_mod, cluster = study_id_1, clubSandwich = TRUE)
  
  
  # save output
  write_rds(mlma_mod, sprintf("mlma_%s.rds", typ))
}
...
plot_equivalence_test(m_file_list = c("mlma_Convenience.rds", "mlma_Community.rds",
