...
### Extend mlma with sig moderators
```{r}

# transform data into an escalc object
rho = .5
dat <- escalc(yi = cor_yi, vi = cor_vi, data = dat)
dat$study_id <- as.character(dat$study_id_1)
dat$es_id <- as.character(dat$es_id)
...
multiplemoderator <- rma.mv(yi ~ 1 + context + cor_type,
                   V = V_mat,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=dat,
                   digits=4)
...
write_rds(multiplemoderator, file = "mlma_multiplemoderator.rds")

### Cannot run equivalence tests due to small study numbers
### Military context has no in between level
### Workplace email has no in between or within levels
