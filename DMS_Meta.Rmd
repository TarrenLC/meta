...
# identify outliers; 2 in this 

caseoutliers <- boxplot(mlma$yi, plot=FALSE)$out
caseoutliers <- round(caseoutliers, digits = 3)

# Debugging prints
rounded_yi <- round(mlma$yi, digits = 3)
print(rounded_yi)
print(caseoutliers)

# Create logical vector for non-outliers
logical_vector <- !(rounded_yi %in% caseoutliers)
print(logical_vector)

# Save non-outliers in a new dataframe
vectoradvice_out <- mlma[logical_vector, ]

# Verify the result
print(vectoradvice_out)

#re-run model with 2 outliers removed

overall <- rma.mv(conduct_mlma(dat = dat, rho = rho)
