...
for (rho in rho_vec) {
  ...
  V_matb <- vcalc(vi = vi,
   ...
   m[[4]] <- rma.mv(yi ~ 1 + culture_powerdistance_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
  ...
  m[[5]] <- rma.mv(yi ~ 1 + culture_individualism_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
  ...
  m[[6]] <- rma.mv(yi ~ 1 + culture_mtas_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
  ...
  m[[7]] <- rma.mv(yi ~ 1 + culture_uncertaintyavoidance_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
  ...
  m[[8]] <- rma.mv(yi ~ 1 + culture_longtermorientation_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
  ...
  m[[9]] <- rma.mv(yi ~ 1 + culture_indulgence_gmc,
                   V = V_matb,
                   random = ~ 1 | study_id_1/es_id, # random effect of study and estimate
                   data=datb,
                   digits=4)
...
### Additional analyses on sig moderators
```{r}

## Splitting by decision context and effect size metric
...
### General/unspecified
df_general <- dat %>% 
  subset(context == "General/Unspecified")


# fitting multilevel model
mlma_gen <- conduct_mlma(dat = df_general, rho = rho)
mlma_gen <- robust(mlma_gen, cluster = study_id_1, clubSandwich = TRUE)
mlma_gen


### Military tactical planning  #### ONLY 2 EFFECT SIZES
df_military <- dat %>% 
  subset(context == "Military tactical planning")


# fitting multilevel model
mlma_m <- conduct_mlma(dat = df_military, rho = rho)
mlma_m <- robust(mlma_m, cluster = study_id_1, clubSandwich = TRUE)
  

### Risky driving
df_risky <- dat %>% 
  subset(context == "Risky driving")


# fitting multilevel model
mlma_rd <- conduct_mlma(dat = df_risky, rho = rho)
mlma_rd <- robust(mlma_rd, cluster = study_id_1, clubSandwich = TRUE)

mlma_rd
  
### Smart phone use
df_phone <- dat %>% 
  subset(context == "Smartphone use")


# fitting multilevel model
mlma_sp <- conduct_mlma(dat = df_phone, rho = rho)
mlma_sp <- robust(mlma_sp, cluster = study_id_1, clubSandwich = TRUE)

mlma_sp

### Stressful situations
df_stress <- dat %>% 
  subset(context == "Stressful situations")


# fitting multilevel model
mlma_sp <- conduct_mlma(dat = df_stress, rho = rho)
mlma_sp <- robust(mlma_sp, cluster = study_id_1, clubSandwich = TRUE)

mlma_sp


### Workplace email #### ONLY 1 EFFECT SIZE #####
df_email <- dat %>% 
  subset(context == "Workplace email use")


# fitting multilevel model
mlma_em <- conduct_mlma(dat = df_email, rho = rho)
mlma_em <- robust(mlma_em, cluster = study_id_1, clubSandwich = TRUE)
mlma_me


# If sig, do equivalence test.
...
res.a1 <- rma(yi, vi, mods = ~ age_diff_gmc + prop_female_gen +
                culture_powerdistance_gmc + culture_individualism_gmc +
                culture_mtas_gmc + culture_uncertaintyavoidance_gmc +
                culture_longtermorientation_gmc + culture_indulgence_gmc +
                dv + context + year_of_publication, data=dat)
...
newdata <- select(dat, study_id_1, es_id, first_author, yi, sei, vi)
...
names1<-unique(names)
data1<-agg(id=study_id_1,es=yi,var=vi,method="BHHR",cor= 1.0,mod=NULL, data=dat)
...
power1 <- mpower(effect_size = 0.11,
                 study_size = 333, # Average of total number of Ps in each study?
                 k = 35,
                 i2 = 0.90,
                 es_type = "r")
summary(power1)
...
power1_plot <- power1_plot + ggtitle("Power analysis with a total sample size of 333 \nper study")
power1_plot
